...
describe Shrine::Plugins::RackResponse do
  ...
  it "returns Content-Length header with size if metadata is missing" do
    ...
    response = uploaded_file.to_rack_response
    assert_equal "7", response[1][CONTENT_LENGTH_HEADER]
  end
  ...
  it "doesn't return Content-Type header if MIME type is unknown" do
    ...
    response = uploaded_file.to_rack_response
    refute response[1].key?(CONTENT_TYPE_HEADER)
  end
  ...
  it "returns Content-Type header from :type" do
    ...
    assert_equal "text/plain; charset=utf-8", response[1][CONTENT_TYPE_HEADER]
  end
  ...
  it "it allows setting Content-Type to application/octet-stream" do
    ...
    response = uploaded_file.to_rack_response
    assert_equal "application/octet-stream", response[1][CONTENT_TYPE_HEADER]
  ...
  it "returns Content-Disposition filename from metadata" do
    ...
    response = uploaded_file.to_rack_response
    assert_equal ContentDisposition.inline("plain.txt"), response[1][CONTENT_DISPOSITION_HEADER]
  end

  it "returns Content-Disposition filename from :filename" do
    uploaded_file = @uploader.upload(fakeio(filename: "plain.txt"))
    response = uploaded_file.to_rack_response(filename: "custom.txt")
    assert_equal ContentDisposition.inline("custom.txt"), response[1][CONTENT_DISPOSITION_HEADER]
  end
  ...
  it "returns Content-Disposition filename with id if metadata is missing" do
    ...
    response = uploaded_file.to_rack_response
    assert_equal ContentDisposition.inline("baz"), response[1][CONTENT_DISPOSITION_HEADER]
  end
  ...
  it "returns Content-Disposition disposition from :disposition" do
    ...
    assert_equal ContentDisposition.attachment(uploaded_file.id), response[1][CONTENT_DISPOSITION_HEADER]
  end
  ...
  it "returns ranged responses when :range is given" do
    ...
    assert_equal "bytes 0-6/7", response[1][CONTENT_RANGE_HEADER]
    assert_equal "7",           response[1][CONTENT_LENGTH_HEADER]
    ...
    assert_equal 206,           response[0]
    assert_equal "bytes 0-2/7", response[1][CONTENT_RANGE_HEADER]
    assert_equal "3",           response[1][CONTENT_LENGTH_HEADER]
    assert_equal "con",         response[2].each { |chunk| break chunk }

    uploaded_file = @uploader.upload(fakeio("content"))
    response = uploaded_file.to_rack_response(range: "bytes=2-4")
    assert_equal 206,           response[0]
    assert_equal "bytes 2-4/7", response[1][CONTENT_RANGE_HEADER]
    assert_equal "3",           response[1][CONTENT_LENGTH_HEADER]
    ...
    assert_equal 206,           response[0]
    assert_equal "bytes 4-6/7", response[1][CONTENT_RANGE_HEADER]
    assert_equal "3",           response[1][CONTENT_LENGTH_HEADER]
  ...
  it "returns ranged responses across multiple chunks" do
    ...
    assert_equal "bytes 0-36863/36864", response[1][CONTENT_RANGE_HEADER]
    assert_equal "36864",               response[1][CONTENT_LENGTH_HEADER]
    yielded_content = ""
    response[2].each { |chunk| yielded_content << chunk }
    assert_equal "a" * 16*1024 + "b" * 16*1024 + "c" * 4*1024, yielded_content
    ...
    assert_equal "bytes 0-20479/36864", response[1][CONTENT_RANGE_HEADER]
    assert_equal "20480",               response[1][CONTENT_LENGTH_HEADER]
    yielded_content = ""
    response[2].each { |chunk| yielded_content << chunk }
    ...
    assert_equal "bytes 12288-20479/36864", response[1][CONTENT_RANGE_HEADER]
    assert_equal "8192", response[1][CONTENT_LENGTH_HEADER]
    yielded_content = ""
    response[2].each { |chunk| yielded_content << chunk }
    ...
    assert_equal "bytes 12288-33791/36864", response[1][CONTENT_RANGE_HEADER]
    assert_equal "21504", response[1][CONTENT_LENGTH_HEADER]
    yielded_content = ""
    response[2].each { |chunk| yielded_content << chunk }
    ...
    assert_equal "bytes 35840-36863/36864", response[1][CONTENT_RANGE_HEADER]
    assert_equal "1024", response[1][CONTENT_LENGTH_HEADER]
    yielded_content = ""
    response[2].each { |chunk| yielded_content << chunk }
  ...
  it "returns Accept-Ranges when :range is given" do
    uploaded_file = @uploader.upload(fakeio)
    response = uploaded_file.to_rack_response
    refute response[1].key?(ACCEPT_RANGES_HEADER)
  ...
  it "makes ETag as unique as possible" do
    ...
    etags << @uploader.class.new(:cache)
      ...
      .to_rack_response[1][ETAG_HEADER]
