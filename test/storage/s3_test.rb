...
describe Shrine::Storage::S3 do
  ...
  # To run the Linter against Shrine::Storage::S3, you need an actual S3 bucket
  # or fascimile thereof. So these tests are not run by default. They are only
  # run if you set certain ENV variables -- in .travis.yml, we DO run, but
  # against a minio server (https://github.com/minio/minio) rather than a real
  # S3 bucket.
  #
  # You can set S3_REAL=true, and set relevant other ENV variables for
  # appropriate S3 access -- or something API compatible with S3, like minio.
  #
  # S3_REAL=true
  #
  # S3_BUCKET
  ...
  describe "with real interaction" do
    ...
    it "passes the linter" do
      assert Shrine::Storage::Linter.new(s3).call
    end

    it "passes the linter with prefix" do
      assert Shrine::Storage::Linter.new(s3(prefix: "uploads")).call
    end
  end if ENV["S3_REAL"]
end
